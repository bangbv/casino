...
public class TLMNDLShowCardsService implements IService {
					...
					if(d!=null){
						...
						if(rs != null){
							if(rs.gameId()==rq.gameId() && !b.isFinish()){
								...
								boolean turnValid = (rq.turnIdx() == b.getTurnIdx()) || rq.isBonDoiThong()==1;
								if(turnValid){
									...
									if(lastPlayCards != null || isFirstTurnOfBattle){
										...
										Card firstCard = isFirstTurnOfRoom?d.getFirstCard():new Card(CardID.Card_None);
										...
										if(cardValid){
											...
											if(b.isRingNew()){
												if(isFirstTurnOfRoom && !hasCardMin)
													cardValid = false;
												else{
