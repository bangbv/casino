...
public class BuyPendingScheduler {
	public boolean running = false;
	public synchronized void setRunning(boolean isRunning){
		running = isRunning;
	}
	private class BuyPendingService extends TimerTask{
		@Override
		public void run() {
	...
	Timer tm = null;
	public BuyPendingScheduler(){
		tm = new Timer();
	}
	public BuyPendingScheduler(long iPERIOD){
		this.iPERIOD = iPERIOD;
		tm = new Timer();
	}
	public void start(){ 
		Lib.getLogger().info(Arrays.asList(iPERIOD, this.getClass().getSimpleName()+".start:..."));
		tm.schedule(new BuyPendingService(), iPERIOD, iPERIOD);
